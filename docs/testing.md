...
- Lint 规则；
- Graph 节点和边转换；
- Model Profile 作用域、选择优先级和状态转换；
- Base URL 规范化、SSRF 拒绝和重定向复查；
- API Key 加密封装、替换、撤销和日志脱敏。
...
- SSE 事件顺序；
- Alembic 从空库升级；
- Ollama 与 OpenAI-compatible Mock 的统一 adapter 契约；
- Profile 连接测试、Job 模型快照和凭据不回显；
- personal/workspace Profile 授权隔离。
...
- Owner/Editor/Viewer；
- 备份恢复后的完整闭环；
- 模型设置、连接测试、默认 Profile 和 Query 模型选择。

### Real-model smoke

- Ollama 健康；如提供测试凭据，再运行外部 OpenAI-compatible smoke；
- Ingest 输出通过 Schema；
- 已知问题带正确引用；
- 未知问题拒答；
- 冲突来源不被静默覆盖。
...
make test-e2e
make test-ollama
make test-model-adapters
```

CI 使用 Mock Ollama/OpenAI-compatible 服务运行 adapter 契约，不要求真实 API Key；`test-ollama` 在指定 Mac Studio/模型服务器运行，外部 API smoke 只在显式提供测试 Secret 时运行。
...
- 重启后数据存在；
- Mock 三栏 UI 和四节点图可交互；
- 默认 Ollama Profile 初始化幂等，API Key 字段不可回显。
...
- Ollama 与 Mock OpenAI-compatible Profile 都能通过连接测试和 Ingest；
- Profile GET、SSE、日志、审计和错误不包含 API Key；
- 未测试、invalid 或 revoked Profile 不能启动任务；
...
- 已知问题正确并带有效引用；
- Query 可以选择 personal/workspace Profile，并记录实际 Provider/Model；
- personal Query 保存到 Wiki 时改用 Workspace 默认 Profile；
- 未知问题拒答；
- FTS、当前页和局部图三种范围可用；
...
- 登录失败和上传限流生效；
- 密码库中不存在明文密码；
- personal Profile 不能被其他用户列出、测试或用于 Query；
- Workspace 切换不会保留无权访问的 Profile 选择。
...
- 审计日志包含成员和页面操作；
- Editor/Viewer 不能创建、替换或设定 Workspace Profile；
- API Key 替换和撤销进入审计，但审计中无明文。
...
- 模型不可用时 Wiki 仍可浏览；
- 外部 API 只允许通过生产 Endpoint 策略访问；
- 凭据密文可恢复，主密钥轮换后 Profile 仍可测试。
...
- Ollama 请求超时；
- 自定义 API 返回 401、403、429、5xx、无效 JSON 和流中断；
- Profile 在 queued/running/retrying 阶段被撤销；
- Worker 在模型返回前退出；
- Worker 在数据库提交前退出；
- Redis 重启；
- PostgreSQL 暂时不可用；
...
- 自定义 Base URL 的 loopback、私网、link-local、DNS rebinding 和重定向 SSRF；
- personal/workspace Profile 的 IDOR；
- API Key 不出现在响应、浏览器状态、日志、异常、SSE、审计或测试快照；
- 外部 API 数据传输提示必须在保存 Profile 前确认。
