...
```text
用户选择 Ollama 或 OpenAI-compatible API
→ 填写 Model ID；自定义 API 另填 Base URL/API Key
→ 后端校验 URL 和权限
→ API Key 认证加密后保存
→ 后端执行连接/能力测试
→ 用户设为个人 Query 默认或 Workspace 写入默认
```

保存后，前端只能看到 `has_credential=true`，不能取回 API Key。连接测试和实际调用都由 API/Worker 发起；浏览器不直连模型服务。

### 任务选模

```mermaid
flowchart TD
    A["创建模型任务"] --> B{"是否写共享 Wiki？"}
    B -- "是" --> C["使用 Workspace 默认 Profile"]
    B -- "否，普通 Query" --> D{"请求是否显式选择可访问 Profile？"}
    D -- "是" --> E["使用所选 personal/workspace Profile"]
    D -- "否" --> F{"是否有个人默认？"}
    F -- "是" --> G["使用个人默认"]
    F -- "否" --> C
    C --> H{"Profile active 且策略允许？"}
    E --> H
    G --> H
    H -- "否" --> I["MODEL_PROFILE_REQUIRED/INVALID"]
    H -- "是" --> J["Job 固化非秘密模型快照并入队"]
```

共享写入包括 Ingest、Lint 自动修复、Query 保存到 Wiki、Overview 更新和 Schema migration。普通 Query 可用个人 Profile，但其回答要写回 Wiki 时必须创建一个使用 Workspace 默认 Profile 的新 Job。

### 凭据和撤销

- personal API Key 只有所有者可以替换或撤销；
- Workspace API Key 只有 Owner 可以替换或撤销；
- API Key 不进入 Job、日志、SSE、审计 metadata 或备份明文；
- Profile 撤销后不接受新任务；已运行调用允许在当前请求边界结束，下一次重试失败并要求重新配置；
- 外部 API Profile 明确记录数据会发送到哪个 Endpoint origin，但不记录完整请求内容。
...
- 文件类型、大小和配额通过校验；
- Workspace 已配置 active 的默认写入 Model Profile；
...
| Profile 缺失、撤销或认证失败 | 不开始模型写入；按错误类型决定是否重试 | 提示测试或更换模型配置 |
...
```mermaid
flowchart TD
    A["提交问题、scope 和可选 Profile"] --> B["授权 Profile，创建 Query 与 queued Job"]
...
用户点击“保存到 Wiki”
→ 选择目标页面或创建新页面
→ 使用 Workspace 默认 Profile 创建独立 update Job
→ LLM 返回结构化变更计划
