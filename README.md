...
项目首先在 Mac Studio 上通过 Ollama 和本地 27B 模型完成闭环验证，同时允许用户切换到自己配置的 OpenAI-compatible API；之后部署到阿里云服务器，并逐步支持多用户与团队协作。
...
- 用户可选择 Mac Ollama 或自定义 OpenAI-compatible API；API Key 只由后端加密保存；
- Query 可使用个人 Model Profile，共享 Wiki 写入使用 Workspace 默认 Profile；
...
| 模型 | Ollama / OpenAI-compatible adapter | 本地模型或用户自备 API 推理 |
...
| MVP 1 | 配置 Ollama/API → Markdown/TXT → Source Summary → Wiki/aliases → 图谱 | 两种 Provider 契约一致、来源不可变、页面不重复、兼容导出可用 |
...
| C：LLM 与 Wiki | Ollama/OpenAI-compatible adapter、RQ Worker、Ingest、Query、Schema 和 Lint |
...
在 macOS 上，Ollama 默认原生运行以使用 Metal；容器通过 `host.docker.internal:11434` 访问。用户也可以在模型设置中填写 OpenAI-compatible Base URL、Model ID 和 API Key，并先执行连接测试。浏览器不会直接连接模型服务，保存后的 API Key 也不会再次显示。

> [!NOTE]
> Mac Ollama 选项适用于网站运行在同一台 Mac，或 Ollama 位于服务器可达的受控网络。部署到阿里云后，服务器不能直接访问访问者电脑上的 `localhost`；当前 MVP 不实现浏览器直连或本地 Connector。
